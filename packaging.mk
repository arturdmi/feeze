...
# NOTE on cross-building: this deliberately packages the *host* architecture
# only. The recorder is generated by `fz -c`, which drives clang with host
# settings and links against a host-architecture libfuzion_rt.so, so amd64 and
# arm64 packages must each be produced on a native machine or runner.
...
# deb and rpm both reject a bare '-' in a version, and both sort '~' *before*
# the empty string, so 0.001~dev correctly precedes a later 0.001 release.
